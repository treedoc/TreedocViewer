import { StringCharSource, CharSource } from 'treedoc'
...
export class Metric {
  ...
  getOrCreateValue(labels: { [key: string]: string }): MetricValue {
    for (const m of this.metricsValues) {
      if (m.matchLabels(labels)) return m
    }
    const ret = new MetricValue()
...
export default class PrometheusParser {
  ...
  parseComment(src: CharSource) {
    if (!src.skipSpacesAndReturns()) return
  ...
  parseMetricLine(src: CharSource): MetricLine {
    const ret = new MetricLine(src.readUntilTerminator('{ '))
    if (src.peek() === '{') {
      src.skip(1)
      while (true) {
        ...
        if (quote !== '"' && quote !== "'") {
          throw src.createParseRuntimeException('missing quote when expecting a string label value')
        }
        ...
        if (sep === '}') break
        if (sep !== ',') {
          throw src.createParseRuntimeException(`expect ',' after label: ${key}=${val}`)
        }
      }
    }
    src.skipChars(' \t')
    ret.value = Number.parseFloat(src.readUntilTerminator(' \n\r'))
    if (src.isEof()) return ret
    if (src.peek() === ' ') {
  ...
  updateCurrentMetric(metricLine: MetricLine) {
    ...
    // No matching, will assume it's a 'count' without a type and help definition
